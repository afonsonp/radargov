...
|---|---|---|---|
| — | Reler as peças pelo modelo quando chega um CE revisto | **Decidido a 03/09/2026 por ele («sim deve se reler»), por fazer.** O `vigiar_pecas()` já traz o CE revisto e avisa, mas o `objecto`, a `equipa` e a `proposta` continuam a ser os que se leram do CE antigo — no 21830/2026 o antigo tinha a referência do produto errada (E1 em vez de E3). Falta disparar o `analisar_pecas()` quando a peça nova é um CE/PC (`papeis_da_peca()`), e decidir se se refaz a leitura toda ou só os campos do documento que mudou. Gasta orçamento do modelo | — |
