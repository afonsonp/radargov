...
| M2 | **A ficha de uma entidade com muitos anúncios leva ~1,3 s** | Medido a 17/09/2026 na base dele: a Santa Casa da Misericórdia de Lisboa tem **1 617 anúncios**, e o número que a ficha mostra custa 1,32 s — é o filtro `nif` do `condicoes()`, cuja segunda metade procura as grafias desse NIF varrendo a tabela larga. **Não se mexeu de propósito**: é exactamente a consulta que a lista corre, e é isso que garante que o número e a ligação dão a mesma população. Optimizar quer dizer optimizar as duas ao mesmo tempo | **Uma sessão**, com medição antes e depois, e o teste do número a proteger a regra |
