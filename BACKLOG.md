...
| R1 | **As Entidades, a ficha da entidade e os estados vazios do redesenho** | O pacote «Radar Gov UI redesign» (`docs/historico/REDESENHO.md`, 17/09/2026) tem cinco ecrãs. Ficaram feitos os dois primeiros — o **Hoje** (§1) e o **Ponto de situação** (§2, rota nova `/situacao`). Faltam: **Entidades** (§3 — abas em vez das quatro listas, a tabela com a fita «connosco» e o comparar de duas entidades), a **ficha da entidade** (§4 — os seis factos no topo, o nosso lado à esquerda e o Portal BASE à direita) e os **estados vazios** (§5, dos três). Nada disto é avaria: os três ecrãs funcionam, o que muda é a forma | **Uma sessão**, com o documento ao lado. Nenhuma decisão dele está em falta — o §3 e o §4 já dizem o que mostrar |
...
| D4 | **Os diagramas de `docs/`** | Seis instantâneos de 16/09/2026, gerados pela skill `archify`. Cinco contradizem o código de hoje (a escada com oito ranhuras em vez de dez e sem setas de retorno, a arquitectura a dizer «única fonte», a recolha sem a Vortal, a porta a atribuir ao CSRF o que é do `origem_e_nossa()`, as peças sem a seta de retorno). **Regenerar é gesto de uma sessão com a skill**, e a fase 5 não o fez | **Uma sessão com a `archify`.** São instantâneos por definição: o que fica é a data no título |
