...
echo
echo " Remove a verificação de hora a hora, a semanal dos contratos e o"
echo " serviço do painel deste computador. A base e a triagem ficam."
echo
read -r -p " Enter para continuar, Ctrl-C para desistir. "
# os de 09h e 17h ficam na lista: uma instalação antiga ainda os pode ter
systemctl --user disable --now radar-hora.timer radar-09h.timer radar-17h.timer radar-contratos.timer radar-painel.service 2>/dev/null
rm -f "$UNIDADES"/radar-hora.timer "$UNIDADES"/radar-09h.timer "$UNIDADES"/radar-17h.timer \
